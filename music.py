...
class MusicPlayer(commands.Cog,name='Music'):
    ...
    async def start_song(self,msg,song):
        ...
        try:
            ...
            print(f'Deleting Files by Exception {e}')
            self.done(msg,msgId.id)
    ...
    @commands.has_permissions(manage_channels=True)
    @command(aliases=['vol'])
    async def volume(self,msg,*args):
        """
        ...
        """
        _display_volume = lambda x: '%s : |%s> %i/%i|\r' % ('Volume', '='*int(30*x), x*200, 200)
        if len(args)> 0:
            vol = int(args[0])
            ...
            if msg.author.voice is not None:
                if msg.voice_client is not None:
                    if msg.voice_client.channel == msg.author.voice.channel and msg.voice_client.is_playing() is True:
                        ...
                        _ = await msg.message.add_reaction(emoji='✅')

                        print(_display_volume(vol/2))

                        emb=discord.Embed(colour=self.random_color, title=_display_volume(vol/2))
                        return await msg.send(embed=emb)
                        # return await msg.channel.send(_display_volume(vol/2))

        else:
            if msg.author.voice is not None:
                if msg.voice_client is not None:
                    if msg.voice_client.channel == msg.author.voice.channel and msg.voice_client.is_playing() is True:
                        print(_display_volume(msg.voice_client.source.volume))

                        emb=discord.Embed(colour=self.random_color, title=_display_volume(msg.voice_client.source.volume))
                        return await msg.send(embed=emb)
                        # return await msg.channel.send(_display_volume(msg.voice_client.source.volume))
